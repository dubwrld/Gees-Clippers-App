...
class NavigationMenu {
    ...
    homeBtn: HTMLImageElement = document.getElementById('index-nav-menu-home-btn') as HTMLImageElement;
    navBTNs: HTMLCollectionOf<Element> = document.getElementsByClassName('navBTN');
    homePageURL: string = 'http://localhost:8080';
    adminPageURL: string = 'http://localhost:8080/admin';
    barberPageURL: string = 'http://localhost:8080/barber'
    ...
    //NOTE - Login form handler
    loginForm = document.getElementById('nav-menu-userLogin-form') as HTMLFormElement;
    loginInput = document.getElementById('nav-menu-username') as HTMLInputElement;
    passwordInput = document.getElementById('nav-menu-password') as HTMLInputElement;

    navMenuClicked = false;

    constructor() {

        // if (this.parsedData.loggedIn) {
        //     this.loginForm.style.display = 'none';
        //     this.accountPageContainer.insertAdjacentHTML("beforeend", `<button style="width:100%; height:20px; color:white; " id="userAccount">Account</button>`);
        //     document.getElementById("userAccount").addEventListener("click", () => { window.location.href = this.adminPageURL });
        // }

        //when image element is clicked it sends the user to the url listed
        this.homeBtn.addEventListener('click', () => {
            window.location.href = this.homePageURL;
        });

        for (let navBtn of this.navBTNs) {
            (navBtn as HTMLElement).addEventListener('mouseenter', () => {
        ...
        `);
            ...
            (navBtn as HTMLElement).addEventListener('focus', () => {
                navBtn.insertAdjacentHTML('afterbegin', `
        <svg fill="#000000" height="30px" width="30px" style="padding:0 5px;"  version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 429.985 429.985" xml:space="preserve">
                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
        ...
        `);
            ...
            (navBtn as HTMLElement).addEventListener('mouseleave', () => {
                document.getElementById('Capa_1').remove();
            });


            (navBtn as HTMLElement).addEventListener('blur', () => {
                document.getElementById('Capa_1').remove();
            });
        }
    ...
    async loginFormHandler(username: string, password: string) {
        ...
        for (let acc of data) {
            ...
            if (username == acc.login && password == acc.password) {
                ...
                if (acc.accountType == "barber") {
                    ...
                    // Convert JSON object to a string and store it in localStorage
                    ...
                    //window.location.href = this.barberPageURL;
                    return;
                    ...
                    this.accountPageContainer.insertAdjacentHTML("beforeend", `<button id="userAccount">Account</button>`);
                    document.getElementById("userAccount").addEventListener("click", () => { window.location.href = this.adminPageURL });
                    // window.location.href = this.adminPageURL;
                    return;
                }
